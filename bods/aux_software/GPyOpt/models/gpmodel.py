...
class GPModel(BOModel):
    ...
    def __init__(self, kernel=None, noise_var=None, exact_feval=False, n_samples = 10, n_burnin = 100, subsample_interval = 10, step_size = 1e-1, leapfrog_steps=20, verbose=False, ARD=True):
        self.kernel = kernel
    ...
    def _create_model(self, X, Y):
        ...
        for i in range(self.n_samples):
            ...
            # --- Define prior on the hyper-parameters for the kernel (for integrated acquisitions)
            #self.model_instances[i].kern.set_prior(GPy.priors.Gamma.from_EV(2.,4.))
            #self.model_instances[i].likelihood.variance.set_prior(GPy.priors.Gamma.from_EV(2.,4.))
    ...
    def updateModel(self, X_all, Y_all, X_new=None, Y_new=None):
        ...
        if self.model is None:
            ...
            for i in range(self.n_samples):
                ...
                #self.model_instances[i].optimize(max_iters = 200)
        ...
        #print(self.hmc_samples)
            ...
            #self.model[:] = self.hmc_samples[0]
        #else:
            #self.model[self.model._fixes_] = self.hmc_samples[0]   
        ...
        for i in range(self.n_samples):
            ...
            #if self.model._fixes_ is None:
                #self.model[:] = self.hmc_samples[i]
            #else:
                #self.model[self.base_model._fixes_] = self.hmc_samples[i]   
            #self.model._trigger_params_changed()
            #self.set_hyperparameters(i)
    ...
    def predict_noiseless(self, X, full_cov=False):
        ...
        """
        if X.ndim==1: X = X[None,:]
        m, v = self.current_model.predict_noiseless(X,full_cov)
        #m += self.constant_mean
        if not full_cov:
            v = np.clip(v, 1e-10, np.inf)
        return m, v
    ...
    def partial_precomputation_for_covariance_conditioned_on_next_point(self, next_point):
        """
        Computes the posterior covariance between points.
        :param X1: some input observations
        :param X2: other input observations
